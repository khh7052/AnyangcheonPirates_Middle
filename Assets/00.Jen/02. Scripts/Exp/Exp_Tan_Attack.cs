...
public class Exp_Tan_Attack : MonoBehaviour
{
    ...
    {
        // 초기 슬라이더 값 설정
        InitializeSlider(amplitudeSlider, 0f, 3f, amplitude, OnAmplitudeChanged);
        InitializeSlider(frequencySlider, 0f, 3f, frequency, OnFrequencyChanged);

        // 공격 시작 코루틴
        StartCoroutine(TanAttackRoutine());
    ...
    IEnumerator AutoIncreaseValues()
    {
        while (true)
        {
            ...
            {
                amplitude = Mathf.Lerp(1f, 3f, elapsedTime / 3f);
            ...
            while (elapsedTime < 3f)
            {
                amplitude = Mathf.Lerp(3f, 1f, elapsedTime / 3f);
                UpdateSliderAndText(amplitudeSlider, amplitudeText, amplitude);
                ToggleKeys(Key_Right, false);
                ToggleKeys(Key_Left, false);
                ToggleKeys(Key_Up, false);
    ...
    void SpawnAttack()
    {
        if (tanAttackPrefab != null && attackPoint != null)
        {
            GameObject attackInstance = Instantiate(tanAttackPrefab, attackPoint.transform.position, Quaternion.identity);
            ...
            if (tanWaveAttack != null)
            {
                ...
                tanWaveAttack.direction = direction;
            }
        }
    }
}
